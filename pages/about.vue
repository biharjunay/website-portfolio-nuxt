<template>
  <div class="p-5 poppins-regular">
    ...
    <template v-if="!loading">
      ...
      <h1 class="poppins-bold text-2xl mt-5">{{ userData.name }} <i
          class="fa-solid fa-badge-check ml-2 text-blue-400"></i></h1>
      <span class="mt-5">{{ userData.position }}</span>
      <p class="mt-5">{{ userData.description }}</p>
    </template>
    <template v-else>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
      <div>
        <div class="rounded-xl divide-y divide-slate-100 bg-gradient-to-r from-slate-900 to-slate-800">
          <a class="p-4 flex cursor-pointer" v-for="(item, index) in certificationsData" :key="index"
             :href="item.certificateUrl" target="_blank">
            <div class="flex-grow">
